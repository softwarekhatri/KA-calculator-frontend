...
const VariantTable: React.FC<VariantTableProps> = ({ metalType, variants, onEdit, onDelete, onAdd }) => {
    const headerColor = metalType === 'Gold' ? 'bg-amber-900/50' : 'bg-slate-700';
    return (
        <div className="w-full">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-slate-200">{metalType} Variants</h3>
                <button
                    onClick={onAdd}
                ...
                <table className="min-w-full divide-y divide-slate-700">
                    <thead className={headerColor}>
                        <tr>
                            ...
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Tunch</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Add-on /10g</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Making</th>
                    ...
                    <tbody className="bg-slate-800 divide-y divide-slate-700">
                        {variants.length > 0 ? variants.map((variant) => (
                            <tr key={variant.id}>
                                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-white">{variant.name}</td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-300">{variant.tunch}%</td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-300">₹{variant.addOnCharges.toLocaleString('en-IN')}</td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-300">₹{variant.makingCharge.toLocaleString('en-IN')}</td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-right">
                                    <div className="flex justify-end gap-3">
                                        <button onClick={() => onEdit(variant)} className="text-blue-400 hover:text-blue-300"><PencilIcon className="w-5 h-5" /></button>
                                        <button onClick={() => onDelete(variant.id)} className="text-red-400 hover:text-red-300"><TrashIcon className="w-5 h-5" /></button>
                                    </div>
                                </td>
                            </tr>
                        )) : (
                            <tr>
                                <td colSpan={5} className="text-center py-4 text-slate-400">No {metalType} variants added yet.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (typeof importedConfig.goldPrice === 'number' &&
      ...
      <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700 shadow-lg">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleImportClick} className="px-3 py-2 text-sm bg-slate-600 text-white rounded-md hover:bg-slate-500 transition">
              Import
            </button>
            <button onClick={handleExport} className="px-3 py-2 text-sm bg-slate-600 text-white rounded-md hover:bg-slate-500 transition">
              Export
            </button>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <div className="flex justify-between items-center">
              <label htmlFor="goldPrice" className="block text-sm font-medium text-slate-300">24 Karat Gold Price (per 10 gram)</label>
              <button 
                  onClick={() => setIsEditingGold(!isEditingGold)}
                  className="text-sm text-amber-400 hover:text-amber-300 font-semibold px-2"
                  aria-label={isEditingGold ? 'Save gold price' : 'Edit gold price'}
              >
                  {isEditingGold ? 'Done' : 'Edit'}
              </button>
            </div>
            <div className="mt-1 relative rounded-md shadow-sm">
                ...
                <input
                    ...
                    onChange={(e) => props.setGoldPrice(parseFloat(e.target.value) || 0)}
          ...
          <div>
            <div className="flex justify-between items-center">
              <label htmlFor="silverPrice" className="block text-sm font-medium text-slate-300">Pure Silver Price (per 10 gram)</label>
               <button 
                  onClick={() => setIsEditingSilver(!isEditingSilver)}
                  className="text-sm text-slate-400 hover:text-slate-300 font-semibold px-2"
                  aria-label={isEditingSilver ? 'Save silver price' : 'Edit silver price'}
              >
                  {isEditingSilver ? 'Done' : 'Edit'}
              </button>
            </div>
            <div className="mt-1 relative rounded-md shadow-sm">
                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                ...
                <input
                    ...
                    onChange={(e) => props.setSilverPrice(parseFloat(e.target.value) || 0)}
