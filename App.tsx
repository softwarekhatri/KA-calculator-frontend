...
const App: React.FC = () => {
  ...
  useEffect(() => {
    backendInstance.get("/price").then((response) => {
      if (response.data) {
        setGoldPrice(
          response.data.find((item: IPriceData) => item.type === "GOLD").price
        );
        setSilverPrice(
          response.data.find((item: IPriceData) => item.type === "SILVER").price
        );
      }
    });
  }, []);
  ...
  // Wrappers to match ConfigSection prop types
  const handleSetGoldPrice = (type: metalType, price: number) => {
    if (type === "GOLD") setGoldPrice(price);
  };
  const handleSetSilverPrice = (price: number) => {
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <main className="container mx-auto px-4 pb-10">
        ...
        <div>
          {activeTab === "config" && (
            <ConfigSection
              goldPrice={{ type: "GOLD", price: goldPrice }}
              setGoldPrice={handleSetGoldPrice}
              silverPrice={{ type: "SILVER", price: silverPrice }}
              setSilverPrice={handleSetSilverPrice}
