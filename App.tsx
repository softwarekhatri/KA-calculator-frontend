...
const App: React.FC = () => {
  ...
  const [goldPrice, setGoldPrice] = useState<number>(122000);
  const [silverPrice, setSilverPrice] = useState<number>(1500);
  const [goldVariants, setGoldVariants] = useState<ItemVariant[]>([
    { id: 'default-750-kdm', name: '750 KDM', tunch: 75, addOnCharges: 4500, makingCharge: 500 },
    { id: 'default-916-kdm', name: '916 KDM', tunch: 91.6, addOnCharges: 4000, makingCharge: 400 },
  ]);
  ...
  const handleSaveVariant = (variantData: Omit<ItemVariant, 'id'>) => {
    if (editingMetalType === 'Gold') {
      setGoldVariants(prev => {
        if (editingVariant) {
          return prev.map(v => v.id === editingVariant.id ? { ...v, ...variantData } : v);
        }
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <main className="container mx-auto px-4 pb-10">
        <div className="border-b border-slate-700 mb-6">
            <nav className="-mb-px flex space-x-2 md:space-x-4 justify-center md:justify-start" aria-label="Tabs">
                <TabButton tabId="config" currentTab={activeTab} onClick={setActiveTab}>Configuration</TabButton>
                <TabButton tabId="gold_calc" currentTab={activeTab} onClick={setActiveTab}>Gold Calculator</TabButton>
                <TabButton tabId="silver_calc" currentTab={activeTab} onClick={setActiveTab}>Silver Calculator</TabButton>
            </nav>
        </div>
